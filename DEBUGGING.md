  ...
  --metric-data-queries '[{
    ...
    "MetricStat":{
      "Metric":{"Namespace":"AWS/Events","MetricName":"TriggeredRules",
        "Dimensions":[{"Name":"RuleName","Value":"'${STAGE}'-s3-input-events"}]},
      "Period":60,"Stat":"Sum"}}]' \
...
aws cloudwatch get-metric-data \
  --metric-data-queries '[{
    ...
    "MetricStat":{
      "Metric":{"Namespace":"AWS/Events","MetricName":"TriggeredRules",
        "Dimensions":[{"Name":"RuleName","Value":"'${STAGE}'-receive-s3-events"}]},
      "Period":60,"Stat":"Sum"}}]' \
  --start-time "$START_TIME" --end-time "$END_TIME" \
...
**Expected Target:**
- RPS Account custom event bus ARN: `arn:aws:events:eu-west-1:222222222222:event-bus/${STAGE}-cross-account-bus`
...
# Describe the rule
aws events describe-rule \
  --name ${STAGE}-receive-s3-events \
  --event-bus-name ${STAGE}-cross-account-bus \
  --region ${REGION} \
  --profile rps-account
...
DLQ_URL=$(aws sqs get-queue-url \
  --queue-name ${STAGE}-processor-dlq \
  --region ${REGION} \
  --profile rps-account \
  --query 'QueueUrl' \
  --output text)
...
aws logs create-log-group \
  --log-group-name /aws/events/${STAGE}-debug \
  --profile rps-account
...
aws events put-targets \
  ...
  --event-bus-name ${STAGE}-cross-account-bus \
  --targets '[{"Id":"debug-logs","Arn":"arn:aws:logs:'${REGION}':'${RPS_ACCOUNT_ID}':log-group:/aws/events/'${STAGE}'-debug"}]' \
  --profile rps-account
...
aws logs tail /aws/events/${STAGE}-debug --profile rps-account
```
...
aws cloudwatch get-metric-data \
  --metric-data-queries '[{"Id":"m1","MetricStat":{"Metric":{"Namespace":"AWS/Events","MetricName":"TriggeredRules","Dimensions":[{"Name":"RuleName","Value":"'${STAGE}'-receive-s3-events"}]},"Period":60,"Stat":"Sum"}}]' \
...
```bash
aws sqs get-queue-attributes \
  --queue-url https://sqs.${REGION}.amazonaws.com/${RPS_ACCOUNT_ID}/${STAGE}-processor-queue \
  --attribute-names Policy \
...
aws events describe-event-bus --name ${STAGE}-cross-account-bus --region ${REGION} --profile rps-account --query 'Policy' --output text | jq '.'

echo -e "\n=== 4. RPS EventBridge Rule ==="
aws events describe-rule --name ${STAGE}-receive-s3-events --event-bus-name ${STAGE}-cross-account-bus --region ${REGION} --profile rps-account

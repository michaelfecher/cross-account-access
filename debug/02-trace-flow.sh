...
if (( $(echo "$STEP1_TRIGGERED > 0" | bc -l) )); then
  ...
  echo "  2. No files uploaded to input/ prefix in the last 10 minutes"
...
if (( $(echo "$STEP2_TRIGGERED > 0" | bc -l) )); then
  if (( $(echo "$STEP2_INVOCATIONS > 0" | bc -l) )) && (( $(echo "$STEP2_FAILED == 0" | bc -l) )); then
    ...
    echo ""
    echo "LIKELY CAUSE: SQS queue policy has aws:SourceAccount condition"
    echo "This blocks cross-account events even though they go through EventBridge."
    echo ""
    echo "NEXT STEPS:"
    echo "  1. Check SQS queue policy: bash debug/04-check-rps.sh"
    echo "  2. Look for 'aws:SourceAccount' condition - should be 'aws:SourceArn' instead"
    echo "  3. See DEBUGGING.md 'SQS Queue Policy Issue' section"
    echo ""
    exit 1
  else

...
echo "     aws s3 ls s3://${OUTPUT_BUCKET_NAME}/ --profile \${CORE_PROFILE}"
echo ""
...
if (( $(echo "$STEP1_TRIGGERED > 0" | bc -l) )); then
  ...
  echo "  1. Input S3 bucket doesn't have EventBridge notifications enabled"
  echo "  2. No files uploaded to input bucket in the last 10 minutes"
...
if (( $(echo "$STEP4_INVOCATIONS > 0" | bc -l) )); then
  if (( $(echo "$STEP4_ERRORS == 0" | bc -l) )); then
    ...
    echo "  Input Bucket → Core EventBridge → RPS Custom Bus → SQS → Lambda → Output Bucket"
    echo ""
    echo "If output file still not created, check:"
    echo "  1. Lambda can AssumeRole to Core S3AccessRole"
    echo "  2. Core S3AccessRole has write permissions to output bucket"
    echo "  3. Lambda logs for errors: aws logs tail /aws/lambda/$LAMBDA_NAME --region $REGION --profile ${RPS_PROFILE:-default}"
    echo "  4. Core S3AccessRole permissions: bash debug/05-check-policies.sh"
    echo ""
  else
    ...
    echo ""
    echo "POSSIBLE CAUSES:"
    echo "  1. Lambda can't AssumeRole Core S3AccessRole (permission denied)"
    echo "  2. Core S3AccessRole can't read from input bucket (permission denied)"
    echo "  3. Core S3AccessRole can't write to output bucket (permission denied)"
    echo "  4. Lambda code error"
    echo ""
    echo "NEXT STEPS:"
    echo "  1. Check Lambda logs:"
    echo "     aws logs tail /aws/lambda/$LAMBDA_NAME --since 10m --region $REGION --profile ${RPS_PROFILE:-default}"
    echo "  2. Verify AssumeRole permissions: bash debug/05-check-policies.sh"
    echo ""
    exit 1
  fi
else
